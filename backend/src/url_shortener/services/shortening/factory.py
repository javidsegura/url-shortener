...
class BaseCreator(ABC):
	...
	async def _store_shortened_url_in_redis(
		self, original_url: str, shortened_url: str, minutes_until_expiration: int
	):
		redis = await self.redis_client.get_client()
		await redis.set(
			name=shortened_url, value=original_url, ex=minutes_until_expiration * 60
		)
		logger.debug(
	...
	async def shorten_url(
		self, original_url: str, minutes_until_expiration: int = 10
	) -> str:
		if not isinstance(original_url, str):
			raise ValueError(
				f"Original url must be an str objeect. Currently is: {type(original_url)}"
			)
		if not (
			self.app_settings.MIN_MINUTES_STORAGE
			<= minutes_until_expiration
			<= self.app_settings.MAX_MINUTES_STORAGE
		):
			raise ValueError(
				f"Minutes until experiation must be >= {self.app_settings.MIN_MINUTES_STORAGE} or <= {self.app_settings.MAX_MINUTES_STORAGE}. Currently is {minutes_until_expiration}"
			)
		shortener = self.factory_method()
		shortened_url = shortener.shorten_url(original_url)
		await self._store_shortened_url_in_redis(
			original_url, shortened_url, minutes_until_expiration
		)
